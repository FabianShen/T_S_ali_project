...
% FD_beijing = FD(:,1:60);
% a = (f_intensity_matrix*e)'.*FD_beijing;
% bm = readmatrix('数据分类.xlsx',Sheet='Sheet1',Range='F3:M44');
% FP_beijing =zeros(42,60);
% for i = 1:42
%     FP_beijing(i,:) = sum(a(i:31:31*42,:),1);
%     Cons_beijing(i,:) = sum(FD_beijing(i:31:31*42,:),1);
% end
% FP_beijing = (FP_beijing'*bm)';
% Cons_beijing = (Cons_beijing'*bm)';
%%
% barh([sum(FP_beijing(:,1:30),2),sum(FP_beijing(:,31:60),2)])
% barh([sum(FP_beijing(:,1:6:60),2),sum(FP_beijing(:,2:6:60),2),sum(FP_beijing(:,3:6:60),2),sum(FP_beijing(:,4:6:60),2),sum(FP_beijing(:,5:6:60),2),sum(FP_beijing(:,6:6:60),2)])
% barh([sum([FP_beijing(:,1:6),FP_beijing(:,31:36)],2),sum([FP_beijing(:,7:12),FP_beijing(:,37:42)],2),sum([FP_beijing(:,13:18),FP_beijing(:,43:48)],2),sum([FP_beijing(:,19:24),FP_beijing(:,49:54)],2),sum([FP_beijing(:,25:30),FP_beijing(:,55:60)],2)])
% pop = repmat([FD_per_sep{1}(1,1:30),FD_per_sep{2}(1,1:30)],42,1);
% [G_f,G_e] = caulgini(FP_beijing',Cons_beijing',pop',sector);
% 
% %%
% pop = repmat([FD_per_sep{1}(1,1:30),FD_per_sep{2}(1,1:30)],8,1);
% [G,gra]=GiniCoeff_plot(pop, FP_beijing,1);
